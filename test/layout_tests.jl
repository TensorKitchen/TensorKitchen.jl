@testset "Join container boundary and point-aware residual" begin
    ...
    A = ManifoldsBase.embed(V, p1) + ManifoldsBase.embed(V, p2)
    model = JoinModel(V, 2, A)
    @test cost(model, p) ≈ 0 atol = 1.0e-14

    q1 = ([0.8], normalize([1.0, 1.0, 0.2]))
    q = TensorKitchen._solver_point(P, (q1, p2))
    @test cost(model, q) ≈ 0.5 * sum(abs2, embed(V, q1) + embed(V, p2) - A)
    ...
    TensorKitchen.rgrad(model, p)
    p.x[1][1][1] += 0.1
    @test cost(model, p) ≈ 0.5 * sum(abs2, embed(V, p.x[1]) + embed(V, p.x[2]) - A)
